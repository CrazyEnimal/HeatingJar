...
// pin A0 - termistor wewnątrz słoika
// pin A1 - termistor przy sterowniku grzałki
// pin A2 - sterowanie temperaturą
// pin A3 - sterowanie wentylatorem termoobiegu
// pin 5 - PWM sterowanie grzałką
// pin 6 - PWM sterowanie wentylatorem termoobiegu
// pin 2 - Nie-PWM sterowanie wentylatorem chłodzenia układu
// pin 3 - Nie-PWM dioda grzania / błędu
// pin 4 - Nie-PWM test-button
...
String buffer;
char buffer_c[10];
int tryb = 1; // tryb grzania; 1 = ręcznie, 2 = automatycznie (serial)
float automat_temperatura = 40.00;
int automat_wartosc_pwm = 128;

float temperatura(int WejscieAnalogowe){
  return float(pgm_read_word(&temps[WejscieAnalogowe-238]))/10;
} // zwraca temperature w C

void serialuj(int typ, float temperatura_a = 0.00, float temperatura_b = 0.00){
  if(typ==1){ // wywal temperature
    if(temperatura_a >= 100.00){
      Serial.print(String("A6") + String(temperatura_a));
    }else{
      Serial.print(String("A5") + String(temperatura_a));
    }
    if(temperatura_b >= 100.00){
      Serial.print(String("B6") + String(temperatura_b) + String("\n"));
    }else{
      Serial.print(String("B5") + String(temperatura_b) + String("\n"));
    }
  }
  if(typ==2){ // wywal błąd przegrzania
    Serial.print("ERR");
  }
}

void overheat_protection() {
  analogWrite(5, 0); // niski stan powoduje zamknięcie mosfetu
  analogWrite(6, 0); // niski stan powoduje otwarcie tranzystora
  serialuj(2);
  while(true){
    digitalWrite(3, HIGH);
    delay(200);
    digitalWrite(3, LOW);
    delay(200);
  } // miganie diody świadczy o przegrzaniu słoika
}

void buffer_run(){
  buffer = Serial.readString();
  buffer.toCharArray(buffer_c, 10);
  if(buffer == ""){
    delay(10);
  }else if(buffer_c[0] == 'S'){ // Set temperature
    automat_temperatura = int(buffer_c[1] - '0')*100 + int(buffer_c[2] - '0')*10 + int(buffer_c[3] - '0')*1;
    Serial.println("OK");
  }else if(buffer_c[0] == 'M'){ // Set mode
    tryb = int(buffer_c[1] - '0');
    if((tryb <=0) || (tryb >= 3)){
      tryb = 1;
      Serial.println("ERR");
    }else{
  ...
  }else if(buffer_c[0] == 'T'){ // Test mode
...
void temperaturuj(){ // literalnie temperaturuj
  if(temperatura(analogRead(A1)) >= 40.00){
    digitalWrite(2, LOW);
  }else{
    digitalWrite(2, HIGH);
  }
  if(tryb == 1){ // manualnie
    analogWrite(6, int(analogRead(A3)/4.02)); // Wentylator termoobiegu
    analogWrite(5, int(analogRead(A2)/4.02)); // Grzałka
  }else{ // tryb == 2 // automatycznie
    if(temperatura(analogRead(A0)) < automat_temperatura){
      if(automat_wartosc_pwm >= 255){
        automat_wartosc_pwm = 255;
      }else{
        automat_wartosc_pwm++;
      }
    }else if(temperatura(analogRead(A0)) > automat_temperatura){
      if(automat_wartosc_pwm <= 0){
        automat_wartosc_pwm = 0;
      }else{
        automat_wartosc_pwm--;
      }
    }
    analogWrite(6, int(analogRead(A3)/4.02)); // Wentylator termoobiegu
    analogWrite(5, automat_wartosc_pwm); // Grzałka
  }
}

void loop() {
  if(temperatura(analogRead(A0)) >= 125.00 || temperatura(analogRead(A1)) >= 50.00){
    overheat_protection();
  }
  serialuj(1, temperatura(analogRead(A0)), temperatura(analogRead(A1)));
  buffer_run();
  temperaturuj();
  //delay(200); // teraz setTimeout sie tym zajmuje
}
